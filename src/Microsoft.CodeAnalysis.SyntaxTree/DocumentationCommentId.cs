...
namespace Microsoft.CodeAnalysis
{
    ...
    public static class DocumentationCommentId
    {
        ...
        public static string CreateDeclarationId(ISymbol symbol)
        {
            ...
            DeclarationGenerator generator = new DeclarationGenerator(builder);
        ...
        public static string CreateReferenceId(ISymbol symbol)
        {
            ...
            StringBuilder builder = new StringBuilder();
            ReferenceGenerator generator = new ReferenceGenerator(builder, typeParameterContext: null);
            generator.Visit(symbol);
            return builder.ToString();
        }
        ...
        public static ImmutableArray<ISymbol> GetSymbolsForDeclarationId(string id, Compilation compilation)
        {
            ...
            List<ISymbol> results = s_symbolListPool.Allocate();
            try
            {
        ...
        private class ReferenceGenerator : SymbolVisitor<bool>
        {
            ...
            public override bool VisitTypeParameter(ITypeParameterSymbol symbol)
            {
                if (!IsInScope(symbol))
                {
                    ...
                    DeclarationGenerator declarer = new DeclarationGenerator(_builder);
                ...
                else
                {
                    ...
                    ISymbol container = symbol.ContainingSymbol?.ContainingSymbol;
                    int b = GetTotalTypeParameterCount(container as INamedTypeSymbol);
                    _builder.Append("`");
            ...
            private bool IsInScope(ITypeParameterSymbol typeParameterSymbol)
            {
                ...
                ISymbol typeParameterDeclarer = typeParameterSymbol.ContainingSymbol;

                for (ISymbol scope = _typeParameterContext; scope != null; scope = scope.ContainingSymbol)
                {
        ...
        private static class Parser
        {
            ...
            {
                char kindChar = PeekNextChar(id, index);
                SymbolKind kind;
                ...
                List<INamespaceOrTypeSymbol> containers = s_namespaceOrTypeListPool.Allocate();
                try
                {
            ...
            private static void ParseTypeSymbol(string id, ref int index, Compilation compilation, ISymbol typeParameterContext, List<ISymbol> results)
            {
                ...
                {
                    List<ISymbol> contexts = s_symbolListPool.Allocate();
                    try
                    {
                        ...
                        if (PeekNextChar(id, index) == ':')
                        {
                            ...
                            foreach (ISymbol context in contexts)
                            {
                                index = startIndex;
                ...
                else
                {
                    ...
                    for (int i = 0; i < results.Count; i++)
                    {
                        ...
                        while (true)
                        {
                            if (PeekNextChar(id, index) == '[')
                            {
                                int bounds = ParseArrayBounds(id, ref index);
            ...
            private static void ParseTypeParameterSymbol(string id, ref int index, ISymbol typeParameterContext, List<ISymbol> results)
            {
                ...
                if (PeekNextChar(id, index) == '`')
                {
                    ...
                    index++;
                    int methodTypeParameterIndex = ReadNextInteger(id, ref index);

                    IMethodSymbol methodContext = typeParameterContext as IMethodSymbol;
                    if (methodContext != null)
                    {
                        int count = methodContext.TypeParameters.Length;
                ...
                else
                {
                    ...
                    int typeParameterIndex = ReadNextInteger(id, ref index);

                    IMethodSymbol methodContext = typeParameterContext as IMethodSymbol;
                    INamedTypeSymbol typeContext = methodContext != null ? methodContext.ContainingType : typeParameterContext as INamedTypeSymbol;
            ...
            private static void ParseNamedTypeSymbol(string id, ref int index, Compilation compilation, ISymbol typeParameterContext, List<ISymbol> results)
            {
                ...
                try
                {
                    ...
                    while (true)
                    {
                        string name = ParseName(id, ref index);
                        ...
                        if (arity != 0 || PeekNextChar(id, index) != '.')
                        {
                            ...
                            {
                                ITypeSymbol[] typeArgs = typeArguments.ToArray();
                                for (int i = 0; i < results.Count; i++)
                                {
            ...
            private static bool ParseTypeArguments(string id, ref int index, Compilation compilation, ISymbol typeParameterContext, List<ITypeSymbol> typeArguments)
            {
                ...
                while (true)
                {
                    ITypeSymbol type = ParseTypeSymbol(id, ref index, compilation, typeParameterContext);
            ...
            {
                ImmutableArray<ISymbol> members = container.GetMembers(memberName);

                foreach (ISymbol symbol in members)
                {
                    if (symbol.Kind == SymbolKind.NamedType)
                    {
                        INamedTypeSymbol namedType = (INamedTypeSymbol)symbol;
            ...
            {
                List<ParameterInfo> parameters = s_parameterListPool.Allocate();
                try
                {
                    int startIndex = index;
                    int endIndex = index;
                    ...
                    for (int i = 0, n = containers.Count; i < n; i++)
                    {
                        ImmutableArray<ISymbol> members = containers[i].GetMembers(memberName);
            ...
            private static bool ParameterMatches(IParameterSymbol symbol, ParameterInfo parameterInfo)
            {
                ...
                ITypeSymbol parameterType = parameterInfo.Type;
            ...
            {
                int containingTypeParameterCount = GetTypeParameterCount(typeSymbol.ContainingType);
                if (n < containingTypeParameterCount)
                {
                    return GetNthTypeParameter(typeSymbol.ContainingType, n);
                }

                int index = n - containingTypeParameterCount;
            ...
            private static bool ParseParameterList(string id, ref int index, Compilation compilation, ISymbol typeParameterContext, List<ParameterInfo> parameters)
            {
                ...
                ParameterInfo? parameter = ParseParameter(id, ref index, compilation, typeParameterContext);
                if (parameter == null)
                {
                    return false;
                }
