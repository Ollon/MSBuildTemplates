    ...
    internal sealed partial class ILBuilder
    {
        ...
        private static void MarkReachableFromBranch(ArrayBuilder<BasicBlock> reachableBlocks, BasicBlock block)
        {
            ...
            if (branchBlock != null)
            {
                ...
                object blockedDest = BlockedBranchDestination(block, branchBlock);
                if (blockedDest == null)
                {
        ...
        private static object BlockedBranchDestinationSlow(ExceptionHandlerScope destHandler, ExceptionHandlerScope srcHandler)
        {
            ...
            while (srcHandler != destHandler)
            {
                ...
                if (srcHandler.Type == ScopeType.Try)
                {
                    ...
                    {
                        object blockedDest = handlerBlock.EnclosingHandler.BlockedByFinallyDestination;
                        if (blockedDest != null)
                        {
                            return blockedDest;
                        }
        ...
        {
            ILBuilder snapshot = (ILBuilder)this.MemberwiseClone();
        ...
        {
#if DEBUG
            Type visType = Type.GetType("Roslyn.Test.Utilities.ILBuilderVisualizer, Roslyn.Test.Utilities", false);
            if (visType != null)
            {
