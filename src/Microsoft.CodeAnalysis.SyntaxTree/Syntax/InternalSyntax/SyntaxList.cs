...
namespace Microsoft.CodeAnalysis.Syntax.InternalSyntax
{
    internal abstract partial class SyntaxList : GreenNode
    {
        ...
        internal static WithThreeChildren List(GreenNode child0, GreenNode child1, GreenNode child2)
        {
            ...
            WithThreeChildren result = new WithThreeChildren(child0, child1, child2);
            if (hash >= 0)
            {
                SyntaxNodeCache.AddNode(result, hash);
            }
        ...
        internal static GreenNode Concat(GreenNode left, GreenNode right)
        {
            ...
            if (leftList != null)
            {
                if (rightList != null)
                {
                    ArrayElement<GreenNode>[] tmp = new ArrayElement<GreenNode>[left.SlotCount + right.SlotCount];
                ...
                {
                    ArrayElement<GreenNode>[] tmp = new ArrayElement<GreenNode>[left.SlotCount + 1];
                    leftList.CopyTo(tmp, 0);
            ...
            {
                ArrayElement<GreenNode>[] tmp = new ArrayElement<GreenNode>[rightList.SlotCount + 1];
                tmp[0].Value = left;
