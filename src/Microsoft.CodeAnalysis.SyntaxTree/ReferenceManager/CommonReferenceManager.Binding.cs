...
namespace Microsoft.CodeAnalysis
{
    internal partial class CommonReferenceManager<TCompilation, TAssemblySymbol>
    {
            ...
            out int corLibraryIndex)
        {
            ...
            ArrayBuilder<AssemblyReferenceBinding[]> referenceBindings = ArrayBuilder<AssemblyReferenceBinding[]>.GetInstance();
            try
            {
                ...
                BoundInputAssembly[] boundInputs = new BoundInputAssembly[referenceBindings.Count];
            ...
            DiagnosticBag resolutionDiagnostics)
        {
            ...
            ArrayBuilder<AssemblyData> implicitAssemblies = ArrayBuilder<AssemblyData>.GetInstance();

            // tracks identities we already asked the resolver to resolve:
            PooledHashSet<AssemblyIdentity> requestedIdentities = PooledHashSet<AssemblyIdentity>.GetInstance();
            ...
            if (previousScriptCompilation != null)
            {
                ...
                foreach (KeyValuePair<AssemblyIdentity, PortableExecutableReference> entry in previousScriptCompilation.GetBoundReferenceManager().GetImplicitlyResolvedAssemblyReferences())
                {
                    previouslyResolvedAssembliesOpt.Add(entry.Key, entry.Value);
                }
            }

            ArrayBuilder<MetadataReference> metadataReferencesBuilder = ArrayBuilder<MetadataReference>.GetInstance();
            ...
            ArrayBuilder<(MetadataReference, ArraySegment<AssemblyReferenceBinding>)> referenceBindingsToProcess = ArrayBuilder<(MetadataReference, ArraySegment<AssemblyReferenceBinding>)>.GetInstance();
            ...
            try
            {
                ...
                {
                    (MetadataReference, ArraySegment<AssemblyReferenceBinding>) referenceAndBindings = referenceBindingsToProcess.Pop();
                    MetadataReference requestingReference = referenceAndBindings.Item1;
                    ArraySegment<AssemblyReferenceBinding> bindings = referenceAndBindings.Item2;

                    foreach (AssemblyReferenceBinding binding in bindings)
                    {
                        ...
                        if (previouslyResolvedAssembliesOpt == null || !previouslyResolvedAssembliesOpt.TryGetValue(binding.ReferenceIdentity, out PortableExecutableReference resolvedReference))
                        {
                        ...
                        AssemblyData data = ResolveMissingAssembly(binding.ReferenceIdentity, resolvedReference, importOptions, resolutionDiagnostics);
                        if (data == null)
                        {
                            continue;
                        }
                        ...
                        MetadataReference existingReference = TryAddAssembly(data.Identity, resolvedReference, index, resolutionDiagnostics, Location.None, assemblyReferencesBySimpleName, supersedeLowerVersions);
                        ...
                        AssemblyReferenceBinding[] referenceBinding = data.BindAssemblyReferences(explicitAssemblies, IdentityComparer);
                ...
                {
                    AssemblyReferenceBinding[] referenceBinding = referenceBindings[bindingsIndex];

                    for (int i = 0; i < referenceBinding.Length; i++)
                    {
                        AssemblyReferenceBinding binding = referenceBinding[i];
            ...
            [Out]ArrayBuilder<(MetadataReference, ArraySegment<AssemblyReferenceBinding>)> result)
        {
            ...
            ImmutableArray<int> explicitModuleToReferenceMap = CalculateModuleToReferenceMap(explicitModules, explicitReferenceMap);

            // add module bindings of assembly being built:
            AssemblyReferenceBinding[] bindingsOfAssemblyBeingBuilt = referenceBindings[0];
            int bindingIndex = totalReferencedAssemblyCount;
            for (int moduleIndex = 0; moduleIndex < explicitModules.Length; moduleIndex++)
            {
                MetadataReference moduleReference = explicitReferences[explicitModuleToReferenceMap[moduleIndex]];
                int moduleBindingsCount = explicitModules[moduleIndex].ReferencedAssemblies.Length;
            ...
            {
                ResolvedReference explicitReferenceMapping = explicitReferenceMap[referenceIndex];
        ...
        private static ImmutableArray<int> CalculateModuleToReferenceMap(ImmutableArray<PEModule> modules, ImmutableArray<ResolvedReference> resolvedReferences)
        {
            ...
            ArrayBuilder<int> result = ArrayBuilder<int>.GetInstance(modules.Length);
        ...
        {
            ArrayBuilder<ResolvedReference> result = ArrayBuilder<ResolvedReference>.GetInstance(references.Length);
        ...
        {
            AssemblyReferenceBinding[] referenceBindingsOfAssemblyBeingBuilt = referenceBindings[0];

            // add implicitly resolved assemblies to the bindings of the assembly being built:
            ArrayBuilder<AssemblyReferenceBinding> bindingsOfAssemblyBeingBuilt = ArrayBuilder<AssemblyReferenceBinding>.GetInstance(referenceBindingsOfAssemblyBeingBuilt.Length + implicitAssemblies.Count);
            ...
            DiagnosticBag diagnostics)
        {
            ...
            AssemblyMetadata assemblyMetadata = metadata as AssemblyMetadata;
            ...
            for (int i = 1; i < totalAssemblies; i++)
            {
                ...
                foreach (TAssemblySymbol candidateAssembly in assemblies[i].AvailableSymbols)
                {
                    ...
                    while (match && candidatesToExamine.Count > 0)
                    {
                        ...
                        AssemblyReferenceBinding[] candidateReferenceBinding = boundInputs[candidateIndex].ReferenceBinding;
        ...
        private static int IndexOfCorLibrary(ImmutableArray<AssemblyData> assemblies, IReadOnlyDictionary<string, List<ReferencedAssemblyIdentity>> assemblyReferencesBySimpleName, bool supersedeLowerVersions)
        {
            ...
            {
                AssemblyData assembly = assemblies[i];
