...
namespace Microsoft.CodeAnalysis
{
    public abstract class CommandLineParser
    {
        ...
        {
            ArrayBuilder<KeyValuePair<string, string>> pathMapBuilder = ArrayBuilder<KeyValuePair<string, string>>.GetInstance();
            if (pathMap.IsEmpty())
            {
                return pathMapBuilder.ToImmutableAndFree();
            }

            foreach (string kEqualsV in pathMap.Split(','))
            {
                string[] kv = kEqualsV.Split('=');
                if (kv.Length != 2)
                {
                    errors.Add(Diagnostic.Create(_messageProvider, _messageProvider.ERR_InvalidPathMap, kEqualsV));
                    continue;
                }
                string from = PathUtilities.TrimTrailingSeparators(kv[0]);
                string to = PathUtilities.TrimTrailingSeparators(kv[1]);
    ...
    && (codepage > 0))
            ...
            IList<Diagnostic> errors)
        {
            ...
            string resolvedDirectoryPath = (directory.Length == 0) ? baseDirectory : FileUtilities.ResolveRelativePath(directory, baseDirectory);
