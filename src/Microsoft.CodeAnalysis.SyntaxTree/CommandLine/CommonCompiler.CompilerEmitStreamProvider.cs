        ...
        private sealed class CompilerEmitStreamProvider : Compilation.EmitStreamProvider
        {
            ...
            public void Close(DiagnosticBag diagnostics)
            {
                ...
                catch (Exception e)
                {
                    ...
                    DiagnosticInfo diagnosticInfo = new DiagnosticInfo(messageProvider, messageProvider.ERR_OutputWriteFailed, _filePath, e.Message);
                    ...
                    catch (IOException e)
                    {
                        ...
                        try
                        {
                            ...
                            else if (e.HResult == eWin32SharingViolation)
                            {
                                ...
                                string newFilePath = Path.Combine(Path.GetDirectoryName(_filePath), Guid.NewGuid().ToString() + "_" + Path.GetFileName(_filePath));
