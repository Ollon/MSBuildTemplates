...
namespace Microsoft.CodeAnalysis
{
    ...
    internal abstract partial class CommonCompiler
    {
        ...
        internal virtual string GetAssemblyFileVersion()
        {
            ...
            {
                string name = $"{Type.GetTypeInfo().Assembly.GetName().Name}.dll";
                string filePath = Path.Combine(_clientDirectory, name);
        ...
        {
            RelativePathResolver pathResolver = new RelativePathResolver(Arguments.ReferencePaths, Arguments.BaseDirectory);
        ...
        {
            MetadataReferenceResolver commandLineReferenceResolver = GetCommandLineMetadataReferenceResolver(touchedFiles);
        ...
        {
            return TryReadFileContent(file, diagnostics, out string discarded);
        }
        ...
        internal SourceText TryReadFileContent(CommandLineSourceFile file, IList<DiagnosticInfo> diagnostics, out string normalizedFilePath)
        {
            ...
            try
            {
                using (FileStream data = OpenFileForReadWithSmallBufferOptimization(filePath))
                {
        ...
        internal EmbeddedText TryReadEmbeddedFileContent(string filePath, IList<Diagnostic> diagnostics)
        {
            try
            {
                using (FileStream stream = OpenFileForReadWithSmallBufferOptimization(filePath))
                {
        ...
        private ImmutableArray<EmbeddedText> AcquireEmbeddedTexts(Compilation compilation, IList<Diagnostic> diagnostics)
        {
            ...
            Dictionary<string, SyntaxTree> embeddedTreeMap = new Dictionary<string, SyntaxTree>(Arguments.EmbeddedFiles.Length);
            OrderedSet<string> embeddedFileOrderedSet = new OrderedSet<string>(Arguments.EmbeddedFiles.Select(e => e.Path));
            ...
            ImmutableArray<EmbeddedText>.Builder embeddedTextBuilder = ImmutableArray.CreateBuilder<EmbeddedText>(embeddedFileOrderedSet.Count);
            foreach (string path in embeddedFileOrderedSet)
            {
                EmbeddedText text;
        ...
        private static IReadOnlySet<string> GetEmbedddedSourcePaths(CommandLineArguments arguments)
        {
            ...
            HashSet<string> set = new HashSet<string>(arguments.EmbeddedFiles.Select(f => f.Path));
        ...
        public StreamErrorLogger GetErrorLogger(TextWriter consoleOutput, CancellationToken cancellationToken)
        {
            ...
            Stream errorLog = OpenFile(Arguments.ErrorLogPath,
                                    consoleOutput,
                                    FileMode.Create,
                                    FileAccess.Write,
        ...
        public virtual int Run(TextWriter consoleOutput, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            {
                int errorCode = MessageProvider.ERR_CompileCancelled;
                if (errorCode > 0)
                {
                    DiagnosticInfo diag = new DiagnosticInfo(MessageProvider, errorCode);
        ...
        internal int RunCore(TextWriter consoleOutput, ErrorLogger errorLogger, CancellationToken cancellationToken)
        {
            ...
            ImmutableArray<AdditionalTextFile> additionalTextFiles = ResolveAdditionalFilesFromArguments(diagnosticInfos, MessageProvider, touchedFilesLogger);
            ...
            try
            {
                ...
                if (!analyzers.IsEmpty)
                {
                    ...
                    AnalyzerOptions analyzerOptions = new AnalyzerOptions(ImmutableArray<AdditionalText>.CastUp(additionalTextFiles));
                ...
                string finalPeFilePath = Path.Combine(Arguments.OutputDirectory, outputName);
                string finalPdbFilePath = Arguments.PdbPath ?? Path.ChangeExtension(finalPeFilePath, ".pdb");
                string finalXmlFilePath = Arguments.DocumentationPath;
                ...
                try
                {
                    ...
                    {
                        Stream sourceLinkStreamOpt = OpenFile(
                    ...
                    CommonPEModuleBuilder moduleBeingBuilt = compilation.CheckOptionsAndCreateModuleBuilder(
                        diagnosticBag,
                    ...
                    if (moduleBeingBuilt != null)
                    {
                        ...
                        try
                        {
                            ...
                            if (success)
                            {
                                ...
                                {
                                    Stream xmlStreamOpt = OpenFile(finalXmlFilePath,
                                                                consoleOutput,
                                ...
                                {
                                    using (Stream win32ResourceStreamOpt = GetWin32Resources(Arguments, compilation, out IEnumerable<DiagnosticInfo> errors))
                                    {
                            ...
                            if (analyzerDriver != null)
                            {
                                ...
                                ImmutableArray<Diagnostic> hostDiagnostics = analyzerDriver.GetDiagnosticsAsync(compilation).Result;
                        ...
                        if (success)
                        {
                            ...
                            CompilerEmitStreamProvider peStreamProvider = new CompilerEmitStreamProvider(this, finalPeFilePath);
                            CompilerEmitStreamProvider pdbStreamProviderOpt = emitPdbFile ? new CompilerEmitStreamProvider(this, finalPdbFilePath) : null;
                    ...
                    ImmutableArray<Diagnostic> compileAndEmitDiagnostics = diagnosticBag.ToReadOnly();
                ...
                foreach (AdditionalTextFile additionalFile in additionalTextFiles)
                {
                ...
                if (Arguments.TouchedFilesPath != null)
                {
                    ...
                    Stream readStream = OpenFile(readFilesPath, consoleOutput, mode: FileMode.OpenOrCreate);
                    Stream writtenStream = OpenFile(writtenFilesPath, consoleOutput, mode: FileMode.OpenOrCreate);
                    ...
                    try
                    {
                        ...
                        using (StreamWriter writer = new StreamWriter(writtenStream))
                        {
        ...
        private static void ReportAnalyzerExecutionTime(TextWriter consoleOutput, AnalyzerDriver analyzerDriver, CultureInfo culture, bool isConcurrentBuild)
        {
            ...
            double totalAnalyzerExecutionTime = analyzerDriver.AnalyzerExecutionTimes.Sum(kvp => kvp.Value.TotalSeconds);
            ...
            IOrderedEnumerable<IGrouping<Assembly, KeyValuePair<DiagnosticAnalyzer, TimeSpan>>> analyzersByAssembly = analyzerDriver.AnalyzerExecutionTimes
            ...
            // Table header
            string analyzerTimeColumn = string.Format("{0,8}", CodeAnalysisResources.AnalyzerExecutionTimeColumnHeader);
            string analyzerPercentageColumn = string.Format("{0,5}", "%");
            string analyzerNameColumn = getFormattedAnalyzerName(CodeAnalysisResources.AnalyzerNameColumnHeader);
            consoleOutput.WriteLine(analyzerTimeColumn + analyzerPercentageColumn + analyzerNameColumn);

            // Table rows grouped by assembly.
            foreach (IGrouping<Assembly, KeyValuePair<DiagnosticAnalyzer, TimeSpan>> analyzerGroup in analyzersByAssembly)
            {
                double executionTime = analyzerGroup.Sum(kvp => kvp.Value.TotalSeconds);
                int percentage = (int)(executionTime * 100 / totalAnalyzerExecutionTime);
                ...
                foreach (KeyValuePair<DiagnosticAnalyzer, TimeSpan> kvp in analyzerGroup.OrderByDescending(kvp => kvp.Value))
                {
        ...
        {
            string key = CreateDeterminismKey(args, rawArgs, baseDirectory, parser);
            string filePath = Path.Combine(args.OutputDirectory, args.OutputFileName + ".key");
        ...
        private static string CreateDeterminismKey(CommandLineArguments args, string[] rawArgs, string baseDirectory, CommandLineParser parser)
        {
            ...
            string name = !string.IsNullOrEmpty(args.OutputFileName)
            ...
            foreach (string current in flattenedArgs)
            {
            ...
            MD5 hash = MD5.Create();
            foreach (CommandLineSourceFile sourceFile in args.SourceFiles)
            {
                ...
                try
                {
                    byte[] bytes = File.ReadAllBytes(sourceFile.Path);
                    byte[] hashBytes = hash.ComputeHash(bytes);
                    string data = BitConverter.ToString(hashBytes);
