        ...
        private sealed class AnalyzerExecutionContext
        {
                ...
                AnalyzerExecutor analyzerExecutor)
            {
                Func<Compilation, Task<HostCompilationStartAnalysisScope>> getTask = comp =>
                {
                    return Task.Run(() =>
                    {
                        HostCompilationStartAnalysisScope compilationAnalysisScope = new HostCompilationStartAnalysisScope(sessionScope);
                ...
                ConditionalWeakTable<Compilation, Task<HostCompilationStartAnalysisScope>>.CreateValueCallback callback = new ConditionalWeakTable<Compilation, Task<HostCompilationStartAnalysisScope>>.CreateValueCallback(getTask);
                ConditionalWeakTable<Compilation, Task<HostCompilationStartAnalysisScope>> compilationActionsCache = GetOrCreateCompilationActionsCache(analyzerExecutor.AnalyzerOptions);
            ...
            public void ClearCompilationScopeMap(AnalyzerOptions analyzerOptions, Compilation compilation)
            {
                lock (_gate)
                {
                    if (_lazyCompilationScopeMap != null && _lazyCompilationScopeMap.TryGetValue(analyzerOptions, out ConditionalWeakTable<Compilation, Task<HostCompilationStartAnalysisScope>> compilationActionsCache))
                    {
                ...
                AnalyzerExecutor analyzerExecutor)
            {
                ...
                {
                    EventHandler<Exception> handler = new EventHandler<Exception>((sender, ex) =>
                    {
