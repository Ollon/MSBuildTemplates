...
namespace Microsoft.CodeAnalysis.Diagnostics
{
    ...
    internal class AnalyzerExecutor
    {
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            ConcurrentDictionary<DiagnosticAnalyzer, TimeSpan> analyzerExecutionTimeMapOpt = logExecutionTime ? new ConcurrentDictionary<DiagnosticAnalyzer, TimeSpan>() : null;
            ...
            where TLanguageKindEnum : struct
        {
            ...
            {
                SyntaxNodeAnalysisContext syntaxNodeContext = new SyntaxNodeAnalysisContext(node, containingSymbol, semanticModel, _analyzerOptions, addDiagnostic,
            ...
            OperationAnalyzerStateData analyzerStateOpt)
        {
            ...
            {
                OperationAnalysisContext operationContext = new OperationAnalysisContext(operation, containingSymbol, semanticModel.Compilation, _analyzerOptions, addDiagnostic, d => IsSupportedDiagnostic(operationAction.Analyzer, d), _cancellationToken);
           ...
           where TNodeStateData : AnalyzerStateData, new()
        {
            ...
            PooledHashSet<TBlockAction> blockEndActions = PooledHashSet<TBlockAction>.GetInstance();
            PooledHashSet<TBlockAction> blockActions = PooledHashSet<TBlockAction>.GetInstance();
            ArrayBuilder<TNodeAction> executableNodeActions = ArrayBuilder<TNodeAction>.GetInstance();
            ArrayBuilder<SyntaxNodeAnalyzerAction<TLanguageKindEnum>> syntaxNodeActions = executableNodeActions as ArrayBuilder<SyntaxNodeAnalyzerAction<TLanguageKindEnum>>;
            ArrayBuilder<OperationAnalyzerAction> operationActions = executableNodeActions as ArrayBuilder<OperationAnalyzerAction>;
            ...
            try
            {
                ...
                foreach (TBlockStartAction startAction in startActions)
                {
                    ...
                    {
                        CodeBlockStartAnalyzerAction<TLanguageKindEnum> codeBlockStartAction = startAction as CodeBlockStartAnalyzerAction<TLanguageKindEnum>;
                        if (codeBlockStartAction != null)
                        {
                            PooledHashSet<CodeBlockAnalyzerAction> codeBlockEndActions = blockEndActions as PooledHashSet<CodeBlockAnalyzerAction>;
                            ...
                            ExecuteAndCatchIfThrows(startAction.Analyzer, () =>
                            {
                                ...
                                AnalyzerCodeBlockStartAnalysisContext<TLanguageKindEnum> blockStartContext = new AnalyzerCodeBlockStartAnalysisContext<TLanguageKindEnum>(startAction.Analyzer,
                        ...
                        {
                            OperationBlockStartAnalyzerAction operationBlockStartAction = startAction as OperationBlockStartAnalyzerAction;
                            if (operationBlockStartAction != null)
                            {
                                PooledHashSet<OperationBlockAnalyzerAction> operationBlockEndActions = blockEndActions as PooledHashSet<OperationBlockAnalyzerAction>;
                                // Catch Exception from the start action.
                                ExecuteAndCatchIfThrows(startAction.Analyzer, () =>
                                {
                                    HostOperationBlockStartAnalysisScope operationBlockScope = new HostOperationBlockStartAnalysisScope();
                                    AnalyzerOperationBlockStartAnalysisContext operationStartContext = new AnalyzerOperationBlockStartAnalysisContext(startAction.Analyzer,
            ...
            if (executableNodeActions.Any())
            {
                if (syntaxNodeActions != null)
                {
                    ...
                    IEnumerable<SyntaxNode> syntaxNodesToAnalyze = (IEnumerable<SyntaxNode>)getNodesToAnalyze(executableBlocks);
            ...
            where TNodeStateData : AnalyzerStateData, new()
        {
            foreach (TBlockAction blockAction in blockActions)
            {
                ...
                {
                    CodeBlockAnalyzerAction codeBlockAction = blockAction as CodeBlockAnalyzerAction;
                    ...
                    {
                        OperationBlockAnalyzerAction operationBlockAction = blockAction as OperationBlockAnalyzerAction;
            ...
            where TLanguageKindEnum : struct
        {
            ...
            PooledDictionary<TLanguageKindEnum, ArrayBuilder<SyntaxNodeAnalyzerAction<TLanguageKindEnum>>> nodeActionsByKind = PooledDictionary<TLanguageKindEnum, ArrayBuilder<SyntaxNodeAnalyzerAction<TLanguageKindEnum>>>.GetInstance();
            foreach (SyntaxNodeAnalyzerAction<TLanguageKindEnum> nodeAction in nodeActions)
            {
                ...
                {
                    if (!nodeActionsByKind.TryGetValue(kind, out ArrayBuilder<SyntaxNodeAnalyzerAction<TLanguageKindEnum>> actionsForKind))
                    {
            ...
            ImmutableDictionary<TLanguageKindEnum, ImmutableArray<SyntaxNodeAnalyzerAction<TLanguageKindEnum>>> map = ImmutableDictionary.CreateRange(tuples);
        ...
        private void ExecuteSyntaxNodeActions<TLanguageKindEnum>(
            ...
            where TLanguageKindEnum : struct
        {
            if (nodeActionsByKind.TryGetValue(getKind(node), out ImmutableArray<SyntaxNodeAnalyzerAction<TLanguageKindEnum>> actionsForKind))
            {
        ...
        internal static ImmutableDictionary<OperationKind, ImmutableArray<OperationAnalyzerAction>> GetOperationActionsByKind(IEnumerable<OperationAnalyzerAction> operationActions)
        {
            ...
            PooledDictionary<OperationKind, ArrayBuilder<OperationAnalyzerAction>> operationActionsByKind = PooledDictionary<OperationKind, ArrayBuilder<OperationAnalyzerAction>>.GetInstance();
            foreach (OperationAnalyzerAction operationAction in operationActions)
            {
                foreach (OperationKind kind in operationAction.Kinds)
                {
                    if (!operationActionsByKind.TryGetValue(kind, out ArrayBuilder<OperationAnalyzerAction> actionsForKind))
                    {
            ...
            ImmutableDictionary<OperationKind, ImmutableArray<OperationAnalyzerAction>> map = ImmutableDictionary.CreateRange(tuples);
        ...
        private void ExecuteOperationActions(
            ...
            OperationAnalyzerStateData analyzerStateOpt)
        {
            if (operationActionsByKind.TryGetValue(operation.Kind, out ImmutableArray<OperationAnalyzerAction> actionsForKind))
            {
        ...
        {
            string analyzerName = analyzer.ToString();
            string title = CodeAnalysisResources.CompilerAnalyzerFailure;
            string messageFormat = CodeAnalysisResources.CompilerAnalyzerThrows;
            string[] messageArguments = new[] { analyzerName, e.GetType().ToString(), e.Message };
            string description = string.Format(CodeAnalysisResources.CompilerAnalyzerThrowsDescription, analyzerName, CreateDiagnosticDescription(info, e));
        ...
        {
            string title = CodeAnalysisResources.AnalyzerDriverFailure;
            string messageFormat = CodeAnalysisResources.AnalyzerDriverThrows;
            string[] messageArguments = new[] { e.GetType().ToString(), e.Message };
...
#pragma warning restore RS0013
        ...
        private static bool TryStartAnalyzingOperationReference(ISymbol symbol, int declarationIndex, DiagnosticAnalyzer analyzer, AnalysisScope analysisScope, AnalysisState analysisStateOpt, out OperationAnalyzerStateData analyzerStateOpt)
        {
            ...
            if (analysisStateOpt.TryStartAnalyzingDeclaration(symbol, declarationIndex, analyzer, out DeclarationAnalyzerStateData declarationAnalyzerStateOpt))
            {
        ...
        internal TimeSpan ResetAnalyzerExecutionTime(DiagnosticAnalyzer analyzer)
        {
            ...
            if (!_analyzerExecutionTimeMapOpt.TryRemove(analyzer, out TimeSpan executionTime))
            {
