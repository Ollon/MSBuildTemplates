...
namespace Microsoft.CodeAnalysis.Diagnostics
{
    ...
    internal sealed class SymbolDeclaredCompilationEvent : CompilationEvent
    {
        ...
        {
            string name = this.Symbol.Name;
            if (name == "") name = "<empty>";
            string loc = DeclaringSyntaxReferences.Length != 0 ? " @ " + String.Join(", ", System.Linq.Enumerable.Select(DeclaringSyntaxReferences, r => r.GetLocation().GetLineSpan())) : null;
