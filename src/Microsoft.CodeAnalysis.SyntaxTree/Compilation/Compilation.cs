    ...
    public abstract partial class Compilation
    {
            ...
            where T : CompilationReference
        {
            ...
            for (int i = 0; i < result.Length; i++)
            {
                ...
                PortableExecutableReference peReference = reference as PortableExecutableReference;
        ...
        public Compilation RemoveReferences(IEnumerable<MetadataReference> references)
        {
            ...
            HashSet<MetadataReference> refSet = new HashSet<MetadataReference>(this.ExternalReferences);
...
#pragma warning restore RS0026 // Do not add multiple public overloads with optional parameters
        ...
        public Stream CreateDefaultWin32Resources(bool versionResource, bool noManifest, Stream manifestContents, Stream iconInIcoFormat)
        {
            ...
            checked
            {
                MemoryStream result = new MemoryStream(1024);
        ...
        {
            BinaryReader reader = new BinaryReader(win32Resources, Encoding.Unicode);

            long initialPosition = win32Resources.Position;
        ...
        internal List<Win32Resource> MakeWin32ResourceList(Stream win32Resources, DiagnosticBag diagnostics)
        {
            ...
            List<Win32Resource> resourceList = new List<Win32Resource>();

            foreach (RESOURCE r in resources)
            {
                Win32Resource result = new Win32Resource(
                    data: r.data,
                    codePage: 0,
            ...
            DiagnosticBag diagnostics)
        {
            ...
            HashSet<string> uniqueResourceNames = new HashSet<string>();
            ...
            if (manifestResources != null && manifestResources.Any())
            {
                ...
                foreach (string fileName in addedModuleNames)
                {
        ...
        internal bool CreateDebugDocuments(DebugDocumentsBuilder documentsBuilder, IEnumerable<EmbeddedText> embeddedTexts, DiagnosticBag diagnostics)
        {
            ...
            {
                ArrayBuilder<Cci.DebugSourceDocument> embeddedDocuments = ArrayBuilder<Cci.DebugSourceDocument>.GetInstance();
                ...
                foreach (EmbeddedText text in embeddedTexts)
                {
                    ...
                    if (existingDoc == null)
                    {
                        Cci.DebugSourceDocument document = new Cci.DebugSourceDocument(
                            normalizedPath,
            ...
            CancellationToken cancellationToken)
        {
            ...
            DebugInformationFormat debugFormat = moduleBeingBuilt.EmitOptions.DebugInformationFormat;
        ...
        internal EmitBaseline SerializeToDeltaStreams(
            ...
            CancellationToken cancellationToken)
        {
            Cci.PdbWriter nativePdbWriterOpt = (moduleBeingBuilt.EmitOptions.DebugInformationFormat != DebugInformationFormat.Pdb) ? null :
                new Cci.PdbWriter(
            ...
            {
                EmitContext context = new EmitContext(moduleBeingBuilt, null, diagnostics);
                Guid encId = Guid.NewGuid();

                try
                {
                    DeltaMetadataWriter writer = new DeltaMetadataWriter(
                        context,
                        MessageProvider,
                        baseline,
                        encId,
    ...
    usedImports.Contains(position);
        ...
        public ImmutableArray<AssemblyIdentity> GetUnreferencedAssemblyIdentities(Diagnostic diagnostic)
        {
            ...
            ArrayBuilder<AssemblyIdentity> builder = ArrayBuilder<AssemblyIdentity>.GetInstance();
