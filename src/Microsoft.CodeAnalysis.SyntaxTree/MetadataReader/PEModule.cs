    ...
    internal sealed class PEModule : IDisposable
    {
        ...
        private sealed class PEHashProvider : CryptographicHashProvider
        {
            ...
            internal unsafe override ImmutableArray<byte> ComputeHash(HashAlgorithm algorithm)
            {
                ...
                using (ReadOnlyUnmanagedMemoryStream stream = new ReadOnlyUnmanagedMemoryStream(_peReader, (IntPtr)block.Pointer, block.Length))
                {
        ...
        internal ImmutableArray<string> GetMetadataModuleNamesOrThrow()
        {
            ...
            try
            {
                foreach (AssemblyFileHandle fileHandle in MetadataReader.AssemblyFiles)
                {
                    AssemblyFile file = MetadataReader.GetAssemblyFile(fileHandle);
        ...
        internal IEnumerable<string> GetReferencedManagedModulesOrThrow()
        {
            ...
            foreach (EntityHandle token in nameTokens)
            {
        ...
        {
            string attributeTypeName = MetadataReader.GetString(nameHandle);
            string attributeTypeNamespaceName = MetadataReader.GetString(namespaceHandle);
        ...
        internal IEnumerable<IGrouping<string, TypeDefinitionHandle>> GroupTypesByNamespaceOrThrow(StringComparer nameComparer)
        {
            ...
            ArrayBuilder<IGrouping<string, TypeDefinitionHandle>> result = new ArrayBuilder<IGrouping<string, TypeDefinitionHandle>>();

            foreach (KeyValuePair<string, ArrayBuilder<TypeDefinitionHandle>> pair in namespaces)
            {
        ...
        internal class TypesByNamespaceSortComparer : IComparer<IGrouping<string, TypeDefinitionHandle>>
        {
            ...
            public int Compare(IGrouping<string, TypeDefinitionHandle> left, IGrouping<string, TypeDefinitionHandle> right)
            {
                ...
                if (result == 0)
                {
                    TypeDefinitionHandle fLeft = left.FirstOrDefault();
                    TypeDefinitionHandle fRight = right.FirstOrDefault();
        ...
        private void GetTypeNamespaceNamesOrThrow(Dictionary<string, ArrayBuilder<TypeDefinitionHandle>> namespaces)
        {
            ...
            Dictionary<NamespaceDefinitionHandle, ArrayBuilder<TypeDefinitionHandle>> namespaceHandles = new Dictionary<NamespaceDefinitionHandle, ArrayBuilder<TypeDefinitionHandle>>(NamespaceHandleEqualityComparer.Singleton);
            ...
            foreach (KeyValuePair<NamespaceDefinitionHandle, ArrayBuilder<TypeDefinitionHandle>> kvp in namespaceHandles)
            {
        ...
        private void GetForwardedTypeNamespaceNamesOrThrow(Dictionary<string, ArrayBuilder<TypeDefinitionHandle>> namespaces)
        {
            ...
            foreach (string typeName in _lazyForwardedTypesToAssemblyIndexMap.Keys)
            {
        ...
        private IdentifierCollection ComputeTypeNameCollection()
        {
            try
            {
                IEnumerable<TypeDefToNamespace> allTypeDefs = GetTypeDefsOrThrow(topLevelOnly: false);
        ...
        private IdentifierCollection ComputeNamespaceNameCollection()
        {
            try
            {
                IEnumerable<TypeDefToNamespace> allTypeIds = GetTypeDefsOrThrow(topLevelOnly: true);
                IEnumerable<string> fullNamespaceNames =
        ...
        internal TypeLayout GetTypeLayout(TypeDefinitionHandle typeDef)
        {
            try
            {
                ...
                TypeDefinition def = MetadataReader.GetTypeDefinition(typeDef);
                ...
                System.Reflection.Metadata.TypeLayout layout = def.GetLayout();
        ...
        {
            AttributeInfo info = FindTargetAttribute(token, AttributeDescription.TupleElementNamesAttribute);
            Debug.Assert(!info.HasValue || info.SignatureIndex == 0 || info.SignatureIndex == 1);
        ...
        private ArrayBuilder<string> ExtractStringValuesFromAttributes(List<AttributeInfo> attrInfos)
        {
            ...
            ArrayBuilder<string> result = ArrayBuilder<string>.GetInstance(attrInfos.Count);

            foreach (AttributeInfo ai in attrInfos)
            {
        ...
        {
            bool result = TryExtractValueFromAttribute(handle, out StringAndInt data, s_attributeStringAndIntValueExtractor);
        ...
        {
            if (CrackStringInAttributeValue(out string message, ref sig) && sig.RemainingBytes >= 1)
            {
        ...
        internal static bool CrackBoolArrayInAttributeValue(out ImmutableArray<bool> value, ref BlobReader sig)
        {
            if (sig.RemainingBytes >= 4)
            {
                ...
                {
                    bool[] boolArray = new bool[arrayLen];
                    for (int i = 0; i < arrayLen; i++)
                    {
        ...
        internal List<AttributeInfo> FindTargetAttributes(EntityHandle hasAttribute, AttributeDescription description)
        {
            ...
            try
            {
                foreach (CustomAttributeHandle attributeHandle in MetadataReader.GetCustomAttributes(hasAttribute))
                {
        ...
        private bool IsNoPiaLocalType(TypeDefinitionHandle typeDef, out AttributeInfo attributeInfo)
        {
            ...
            try
            {
                foreach (CustomAttributeHandle attributeHandle in MetadataReader.GetCustomAttributes(typeDef))
                {
        ...
        internal AssemblyReferenceHandle GetAssemblyRef(string assemblyName)
        {
            ...
            try
            {
                ...
                foreach (AssemblyReferenceHandle assemblyRef in MetadataReader.AssemblyReferences)
                {
            ...
            out EntityHandle attributeCtor)
        {
            try
            {
                ...
                {
                    MethodDefinition methodDef = metadataReader.GetMethodDefinition((MethodDefinitionHandle)attributeCtor);
        ...
        private static bool GetAttributeNamespaceAndName(MetadataReader metadataReader, EntityHandle typeDefOrRef, out StringHandle namespaceHandle, out StringHandle nameHandle)
        {
            ...
            try
            {
                ...
                {
                    TypeDefinition def = metadataReader.GetTypeDefinition((TypeDefinitionHandle)typeDefOrRef);
                ...
                try
                {
                    foreach (CustomAttributeHandle attributeHandle in MetadataReader.CustomAttributes)
                    {
                        ...
                        if (signatureIndex != -1)
                        {
                            ...
                            TypeDefinitionHandle parent = (TypeDefinitionHandle)MetadataReader.GetCustomAttribute(attributeHandle).Parent;
        ...
        {
            MethodSpecification methodSpec = MetadataReader.GetMethodSpecification(handle);
        ...
        {
            MethodImplementation impl = MetadataReader.GetMethodImplementation(methodImpl);
        ...
        internal DllImportData GetDllImportData(MethodDefinitionHandle methodDef)
        {
            try
            {
                MethodImport methodImport = MetadataReader.GetMethodDefinition(methodDef).GetImport();
        ...
        internal ConstantValue GetParamDefaultValue(ParameterHandle param)
        {
            ...
            try
            {
                ConstantHandle constantHandle = MetadataReader.GetParameter(param).GetDefaultValue();
        ...
        internal ConstantValue GetConstantFieldValue(FieldDefinitionHandle fieldDef)
        {
            ...
            try
            {
                ConstantHandle constantHandle = MetadataReader.GetFieldDefinition(fieldDef).GetDefaultValue();
        ...
        internal UnmanagedType GetMarshallingType(EntityHandle fieldOrParameterToken)
        {
            try
            {
                BlobHandle blob = GetMarshallingDescriptorHandleOrThrow(fieldOrParameterToken);
        ...
        {
            Constant constantRow = MetadataReader.GetConstant(handle);
        ...
        internal (int FirstIndex, int SecondIndex) GetAssemblyRefsForForwardedType(string fullName, bool ignoreCase, out string matchedName)
        {
            ...
            if (ignoreCase)
            {
                ...
                foreach (KeyValuePair<string, (int FirstIndex, int SecondIndex)> pair in _lazyForwardedTypesToAssemblyIndexMap)
                {
        ...
        private void EnsureForwardTypeToAssemblyMap()
        {
            ...
            {
                Dictionary<string, (int FirstIndex, int SecondIndex)> typesToAssemblyIndexMap = new Dictionary<string, (int FirstIndex, int SecondIndex)>();

                try
                {
                    ExportedTypeHandleCollection forwarders = MetadataReader.ExportedTypes;
                    foreach (ExportedTypeHandle handle in forwarders)
                    {
