        ...
        where Symbol : class, ISymbol
    {
        ...
        private TypeSymbol GetTypeOfTypeSpec(TypeSpecificationHandle typeSpec)
        {
            ...
            try
            {
                ...
                ptype = DecodeTypeOrThrow(ref memoryReader, out bool refersToNoPiaLocalType);
            }
        ...
        private TypeSymbol DecodeTypeOrThrow(ref BlobReader ppSig, SignatureTypeCode typeCode, out bool refersToNoPiaLocalType)
        {
            ...
            switch (typeCode)
            {
                ...
                case SignatureTypeCode.Array:
                    ...
                    {
                        ArrayBuilder<int> builder = ArrayBuilder<int>.GetInstance(countOfSizes);

                        for (int i = 0; i < countOfSizes; i++)
                        {
                            if (ppSig.TryReadCompressedInteger(out int size))
                            {
                                builder.Add(size);
                            }
                            else
                    ...
                    else
                    {
                        ...
                        {
                            if (ppSig.TryReadCompressedSignedInteger(out int lowerBound))
                            {
                                if (lowerBound != 0)
                                {
                                    if (builder == null)
        ...
        private TypeSymbol DecodeGenericTypeInstanceOrThrow(ref BlobReader ppSig, out bool refersToNoPiaLocalType)
        {
            ...
            if (!ppSig.TryReadCompressedInteger(out int argumentCount))
            {
                throw new UnsupportedSignatureContent();
            }
            ...
            ArrayBuilder<KeyValuePair<TypeSymbol, ImmutableArray<ModifierInfo<TypeSymbol>>>> argumentsBuilder = ArrayBuilder<KeyValuePair<TypeSymbol, ImmutableArray<ModifierInfo<TypeSymbol>>>>.GetInstance(argumentCount);
            ArrayBuilder<bool> argumentRefersToNoPiaLocalTypeBuilder = ArrayBuilder<bool>.GetInstance(argumentCount);

            for (int argumentIndex = 0; argumentIndex < argumentCount; argumentIndex++)
            {
                ImmutableArray<ModifierInfo<TypeSymbol>> modifiers = DecodeModifiersOrThrow(ref ppSig, out SignatureTypeCode typeCode);
                argumentsBuilder.Add(KeyValuePair.Create(DecodeTypeOrThrow(ref ppSig, typeCode, out bool argumentRefersToNoPia), modifiers));
        ...
        private TypeSymbol GetTypeOfTypeRef(TypeReferenceHandle typeRef, out bool isNoPiaLocalType)
        {
            ...
            if (cache != null && cache.TryGetValue(typeRef, out TypeSymbol result))
            {
            ...
            try
            {
                Module.GetTypeRefPropsOrThrow(typeRef, out string name, out string @namespace, out EntityHandle resolutionScope);
        ...
        private TypeSymbol GetTypeOfTypeDef(TypeDefinitionHandle typeDef, out bool isNoPiaLocalType, bool isContainingType)
        {
            try
            {
                ...
                if (cache != null && cache.TryGetValue(typeDef, out TypeSymbol result))
                {
...
out string identifier))
        ...
        internal ImmutableArray<LocalInfo<TypeSymbol>> DecodeLocalSignatureOrThrow(ref BlobReader signatureReader)
        {
            ...
            GetSignatureCountsOrThrow(ref signatureReader, signatureHeader, out int localCount, out int typeParameterCount);
            Debug.Assert(typeParameterCount == 0);

            ArrayBuilder<LocalInfo<TypeSymbol>> locals = ArrayBuilder<LocalInfo<TypeSymbol>>.GetInstance(localCount);
            ArrayBuilder<int> offsets = ArrayBuilder<int>.GetInstance(localCount);
            try
            {
                ...
                for (int i = 0; i < localCount; i++)
                {
                    ...
                    byte[] signature = signatureReader.ReadBytes(n);
        ...
        internal LocalInfo<TypeSymbol> DecodeLocalVariableOrThrow(ref BlobReader signatureReader)
        {
            ...
            ImmutableArray<ModifierInfo<TypeSymbol>> customModifiers = DecodeModifiersOrThrow(ref signatureReader, out SignatureTypeCode typeCode);
            ...
            else
            {
                try
                {
                    typeSymbol = DecodeTypeOrThrow(ref signatureReader, typeCode, out bool refersToNoPiaLocalType);
                }
                catch (UnsupportedSignatureContent)
                {
                    typeSymbol = GetUnsupportedMetadataTypeSymbol();
        ...
        internal void DecodeLocalConstantBlobOrThrow(ref BlobReader sigReader, out TypeSymbol type, out ConstantValue value)
        {
            ...
            ImmutableArray<ModifierInfo<TypeSymbol>> customModifiers = DecodeModifiersOrThrow(ref sigReader, out SignatureTypeCode typeCode);

            if (typeCode == SignatureTypeCode.TypeHandle)
            {
                // TypeDefOrRefOrSpec encoded
                type = GetSymbolForTypeHandleOrThrow(sigReader.ReadTypeHandle(), out bool refersToNoPiaLocalType, allowTypeSpec: true, requireShortForm: true);
            ...
            {
                value = DecodePrimitiveConstantValue(ref sigReader, typeCode, out bool isEnumTypeCode);
        ...
        internal bool TryGetLocals(MethodDefinitionHandle handle, out ImmutableArray<LocalInfo<TypeSymbol>> localInfo)
        {
            try
            {
                ...
                MethodBodyBlock methodBody = Module.GetMethodBodyOrThrow(handle);

                if (!methodBody.LocalSignature.IsNil)
                {
                    BlobHandle signatureHandle = Module.MetadataReader.GetStandaloneSignature(methodBody.LocalSignature).Signature;
                    BlobReader signatureReader = Module.GetMemoryReaderOrThrow(signatureHandle);
        ...
        internal unsafe TypeSymbol DecodeLocalVariableTypeOrThrow(ImmutableArray<byte> signature)
        {
            ...
            {
                BlobReader blobReader = new BlobReader(ptr, signature.Length);
        ...
        private void DecodeParameterOrThrow(ref BlobReader signatureReader, /*out*/ ref ParamInfo<TypeSymbol> info)
        {
            ...
            info.CustomModifiers = DecodeModifiersOrThrow(ref signatureReader, out SignatureTypeCode typeCode);
            ...
            info.Type = DecodeTypeOrThrow(ref signatureReader, typeCode, out bool refersToNoPiaLocalType);
        }
        ...
        internal ParamInfo<TypeSymbol>[] GetSignatureForMethod(MethodDefinitionHandle methodDef, out SignatureHeader signatureHeader, out BadImageFormatException metadataException, bool setParamHandles = true)
        {
            ...
            try
            {
                ...
                //CONSIDER: expose to caller?
                paramInfo = DecodeSignatureParametersOrThrow(ref signatureReader, signatureHeader, out int typeParameterCount);
        ...
        {
            BlobHandle signature = module.GetMethodSignatureOrThrow(methodDef);
            BlobReader signatureReader = DecodeSignatureHeaderOrThrow(module, signature, out SignatureHeader signatureHeader);
        ...
        internal SignatureHeader GetSignatureHeaderForProperty(PropertyDefinitionHandle handle)
        {
            ...
            {
                BlobHandle signature = Module.GetPropertySignatureOrThrow(handle);
                DecodeSignatureHeaderOrThrow(signature, out SignatureHeader signatureHeader);
        ...
        private void DecodeCustomAttributeParameterTypeOrThrow(ref BlobReader sigReader, out SerializationTypeCode typeCode, out TypeSymbol type, out SerializationTypeCode elementTypeCode, out TypeSymbol elementType, bool isElementType)
        {
            ...
            if (paramTypeCode == SignatureTypeCode.SZArray)
            {
                ...
                DecodeCustomAttributeParameterTypeOrThrow(ref sigReader, out elementTypeCode, out elementType, out SerializationTypeCode unusedElementTypeCode, out TypeSymbol unusedElementType, isElementType: true);
        ...
        private void DecodeCustomAttributeFieldOrPropTypeOrThrow(ref BlobReader argReader, out SerializationTypeCode typeCode, out TypeSymbol type, out SerializationTypeCode elementTypeCode, out TypeSymbol elementType, bool isElementType)
        {
            ...
            if (typeCode == SerializationTypeCode.SZArray)
            {
                ...
                DecodeCustomAttributeFieldOrPropTypeOrThrow(ref argReader, out elementTypeCode, out elementType, out SerializationTypeCode unusedElementTypeCode, out TypeSymbol unusedElementType, isElementType: true);
                type = GetSZArrayTypeSymbol(elementType, customModifiers: default(ImmutableArray<ModifierInfo<TypeSymbol>>));
                return;
            }
        ...
        {
            DecodeCustomAttributeParameterTypeOrThrow(ref sigReader, out SerializationTypeCode typeCode, out TypeSymbol type, out SerializationTypeCode elementTypeCode, out TypeSymbol elementType, isElementType: false);
        ...
        private TypedConstant DecodeCustomAttributeElementOrThrow(ref BlobReader argReader, SerializationTypeCode typeCode, TypeSymbol type)
        {
            if (typeCode == SerializationTypeCode.TaggedObject)
            {
                ...
                DecodeCustomAttributeFieldOrPropTypeOrThrow(ref argReader, out typeCode, out type, out SerializationTypeCode elementTypeCode, out TypeSymbol elementType, isElementType: false);
        ...
        private KeyValuePair<string, TypedConstant> DecodeCustomAttributeNamedArgumentOrThrow(ref BlobReader argReader)
        {
            ...
            CustomAttributeNamedArgumentKind kind = (CustomAttributeNamedArgumentKind)argReader.ReadCompressedInteger();
            if (kind != CustomAttributeNamedArgumentKind.Field && kind != CustomAttributeNamedArgumentKind.Property)
            {
                throw new UnsupportedSignatureContent();
            }

            DecodeCustomAttributeFieldOrPropTypeOrThrow(ref argReader, out SerializationTypeCode typeCode, out TypeSymbol type, out SerializationTypeCode elementTypeCode, out TypeSymbol elementType, isElementType: false);

            if (!PEModule.CrackStringInAttributeValue(out string name, ref argReader))
            {
                throw new UnsupportedSignatureContent();
            }
            ...
            out KeyValuePair<string, TypedConstant>[] namedArgs)
        {
            try
            {
                ...
                if (Module.GetTypeAndConstructor(handle, out EntityHandle attributeType, out EntityHandle ctor))
                {
        ...
        internal bool GetCustomAttributeWellKnownType(CustomAttributeHandle handle, out WellKnownType wellKnownAttribute)
        {
            ...
            try
            {
                ...
                if (!Module.GetAttributeNamespaceAndName(attributeType, out StringHandle namespaceHandle, out StringHandle nameHandle))
                {
                    return false;
                }
        ...
        {
            BlobReader signatureReader = DecodeSignatureHeaderOrThrow(signature, out SignatureHeader signatureHeader);
            ...
            TypeSymbol[] result = new TypeSymbol[argumentCount];
            for (int i = 0; i < result.Length; i++)
            {
                result[i] = DecodeTypeOrThrow(ref signatureReader, out bool refersToNoPiaLocalType);
            }
        ...
        {
            GetSignatureCountsOrThrow(ref signatureReader, signatureHeader, out int paramCount, out typeParameterCount);
        ...
        internal ImmutableArray<MethodSymbol> GetExplicitlyOverriddenMethods(TypeDefinitionHandle implementingTypeDef, MethodDefinitionHandle implementingMethodDef, TypeSymbol implementingTypeSymbol)
        {
            ...
            try
            {
                ...
                {
                    Module.GetMethodImplPropsOrThrow(methodImpl, out EntityHandle methodDebugHandle, out EntityHandle implementedMethodHandle);
            ...
            try
            {
                switch (token.Kind)
                {
                    ...
                    case HandleKind.MethodSpecification:
                        ...
                        MethodSymbol genericDefinition = (MethodSymbol)GetSymbolForILToken(method);
