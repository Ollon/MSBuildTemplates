...
namespace Microsoft.CodeAnalysis.Text
{
    ...
    public abstract class SourceText
    {
        ...
        private static string Decode(Stream stream, Encoding encoding, out Encoding actualEncoding)
        {
            ...
            using (StreamReader reader = new StreamReader(stream, encoding, detectEncodingFromByteOrderMarks: true, bufferSize: Math.Min(4096, length), leaveOpen: true))
            {
                string text = reader.ReadToEnd();
        ...
        public virtual SourceText WithChanges(IEnumerable<TextChange> changes)
        {
            ...
            foreach (TextChange change in changes)
            {
                ...
                if (newTextLength > 0)
                {
                    SourceText segment = SourceText.From(change.NewText, this.Encoding, this.ChecksumAlgorithm);
        ...
        public virtual IReadOnlyList<TextChange> GetTextChanges(SourceText oldText)
        {
            ...
            List<TextChangeRange> ranges = this.GetChangeRanges(oldText).ToList();
