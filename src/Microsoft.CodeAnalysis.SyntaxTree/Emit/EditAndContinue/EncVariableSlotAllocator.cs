...
namespace Microsoft.CodeAnalysis.Emit
{
    internal sealed class EncVariableSlotAllocator : VariableSlotAllocator
    {
            ...
            ImmutableArray<TypedConstant> tupleElementNames)
        {
            ...
            EncLocalInfo localKey = new EncLocalInfo(new LocalSlotDebugInfo(kind, previousId), previousType, constraints, signature: null);
            ...
            out int slotIndex)
        {
            ...
            // TODO (bug #781309): Should report a warning if the type of the local has changed
            // and the previous value will be dropped.
            EncHoistedLocalInfo localKey = new EncHoistedLocalInfo(new LocalSlotDebugInfo(synthesizedKind, previousId), previousType);
    ...
    _lambdaMapOpt.TryGetValue(syntaxOffset, out KeyValuePair<DebugId, int> idAndClosureOrdinal))
            {
