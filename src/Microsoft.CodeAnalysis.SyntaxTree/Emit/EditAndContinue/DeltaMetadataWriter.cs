...
namespace Microsoft.CodeAnalysis.Emit
{
    internal sealed class DeltaMetadataWriter : MetadataWriter
    {
        ...
        {
            int[] sizes = new int[MetadataTokens.TableCount];
        ...
        {
            CommonPEModuleBuilder moduleBuilder = (CommonPEModuleBuilder)this.module;

            Dictionary<int, AddedOrChangedMethodInfo> addedOrChangedMethodsByIndex = new Dictionary<int, AddedOrChangedMethodInfo>();
            ...
            ImmutableArray<int> deltaTableSizes = this.GetDeltaTableSizes(metadataSizes.RowCounts);
            int[] tableSizes = new int[MetadataTokens.TableCount];
            ...
            ImmutableDictionary<ITypeDefinition, ImmutableArray<ITypeDefinitionMember>> synthesizedMembers = (baseline.Ordinal == 0) ? moduleBuilder.GetSynthesizedMembers() : baseline.SynthesizedMembers;
        ...
        {
            IPEDeltaAssemblyBuilder module = (IPEDeltaAssemblyBuilder)this.module;
        ...
        protected override void CreateIndicesForNonTypeMembers(ITypeDefinition typeDef)
        {
            ...
            bool ok = _typeDefs.TryGetValue(typeDef, out int typeIndex);
            Debug.Assert(ok);
            ...
            ArrayBuilder<int> implementingMethods = ArrayBuilder<int>.GetInstance();
            ...
            {
                IMethodDefinition methodDef = (IMethodDefinition)methodImpl.ImplementingMethod.AsDefinition(this.Context);
                ...
                MethodImplKey key = new MethodImplKey(methodDefIndex, index: 1);
            ...
            foreach (int methodDefIndex in implementingMethods)
            {
                int index = 1;
                while (true)
                {
                    MethodImplKey key = new MethodImplKey(methodDefIndex, index);
                    if (!_methodImpls.TryGetValue(key, out int methodImplIndex))
                    {
        ...
        protected override StandaloneSignatureHandle SerializeLocalVariablesSignature(IMethodBody body)
        {
            ...
            ArrayBuilder<EncLocalInfo> encInfos = ArrayBuilder<EncLocalInfo>.GetInstance();
            ...
            IMethodDefinition method = body.MethodDefinition;
            if (!method.IsImplicitlyDeclared)
            {
                AddedOrChangedMethodInfo info = new AddedOrChangedMethodInfo(
                    body.MethodId,
            ...
            where T : ITypeDefinitionMember
        {
            foreach (T member in index.GetRows())
            {
                if (index.IsAddedNotChanged(member))
                {
                    ...
                    bool ok = map.TryGetValue(typeIndex, out int mapRowId);
                    Debug.Assert(ok);
        ...
        {
            int parameterFirstId = _parameterDefs.FirstRowId;
        ...
        protected override void PopulateEncMapTableRows(ImmutableArray<int> rowCounts)
        {
            ...
            ArrayBuilder<EntityHandle> tokens = ArrayBuilder<EntityHandle>.GetInstance();
            ImmutableArray<int> previousSizes = _previousGeneration.TableSizes;
            ImmutableArray<int> deltaSizes = this.GetDeltaTableSizes(rowCounts);
            ...
            {
                ArrayBuilder<EntityHandle> debugTokens = ArrayBuilder<EntityHandle>.GetInstance();
            ...
            TableIndex[] handledTables = new TableIndex[]
            {
                TableIndex.Module,
                TableIndex.TypeRef,
                TableIndex.TypeDef,
        ...
        {
            foreach (int typeId in _eventMap.GetRows())
            {
        ...
        {
            foreach (int typeId in _propertyMap.GetRows())
            {
        ...
        private sealed class DeltaReferenceIndexer : ReferenceIndexer
        {
            ...
            public override void Visit(Cci.MethodImplementation methodImplementation)
            {
                ...
                IMethodDefinition methodDef = (IMethodDefinition)methodImplementation.ImplementingMethod.AsDefinition(this.Context);
