...
namespace Microsoft.CodeAnalysis.Emit
{
    internal sealed class SymbolChanges
    {
        ...
        {
            ISynthesizedMethodBodyImplementationSymbol synthesizedDef = def as ISynthesizedMethodBodyImplementationSymbol;
            if (synthesizedDef != null)
            {
                Debug.Assert(synthesizedDef.Method != null);

                IMethodSymbol generator = synthesizedDef.Method;
                ISymbol synthesizedSymbol = (ISymbol)synthesizedDef;
        ...
        {
            if (_changes.TryGetValue(symbol, out SymbolChange change))
            {
                return change;
            }
            ...
            switch (change)
            {
                ...
                case SymbolChange.ContainsChanges:
                    IDefinition definition = symbol as IDefinition;
        ...
        {
            Dictionary<ISymbol, SymbolChange> changes = new Dictionary<ISymbol, SymbolChange>();
