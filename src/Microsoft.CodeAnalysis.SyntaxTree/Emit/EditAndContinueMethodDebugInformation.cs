...
namespace Microsoft.CodeAnalysis.Emit
{
    ...
    public struct EditAndContinueMethodDebugInformation
    {
        ...
        public static EditAndContinueMethodDebugInformation Create(ImmutableArray<byte> compressedSlotMap, ImmutableArray<byte> compressedLambdaMap)
        {
            ...
            UncompressLambdaMap(compressedLambdaMap, out int methodOrdinal, out ImmutableArray<ClosureDebugInfo> closures, out ImmutableArray<LambdaDebugInfo> lambdas);
        ...
        private unsafe static ImmutableArray<LocalSlotDebugInfo> UncompressSlotMap(ImmutableArray<byte> compressedSlotMap)
        {
            ...
            ArrayBuilder<LocalSlotDebugInfo> mapBuilder = ArrayBuilder<LocalSlotDebugInfo>.GetInstance();
            int syntaxOffsetBaseline = -1;

            fixed (byte* compressedSlotMapPtr = &compressedSlotMap.ToArray()[0])
            {
                BlobReader blobReader = new BlobReader(compressedSlotMapPtr, compressedSlotMap.Length);
                while (blobReader.RemainingBytes > 0)
                {
                    try
                    {
                        ...
                        SynthesizedLocalKind kind = (SynthesizedLocalKind)((b & 0x3f) - 1);
            ...
            out ImmutableArray<LambdaDebugInfo> lambdas)
        {
            ...
            ArrayBuilder<ClosureDebugInfo> closuresBuilder = ArrayBuilder<ClosureDebugInfo>.GetInstance();
            ArrayBuilder<LambdaDebugInfo> lambdasBuilder = ArrayBuilder<LambdaDebugInfo>.GetInstance();

            fixed (byte* blobPtr = &compressedLambdaMap.ToArray()[0])
            {
                BlobReader blobReader = new BlobReader(blobPtr, compressedLambdaMap.Length);
                try
                {
                    // Note: integer operations below can't overflow since compressed integers are in range [0, 0x20000000)
                    ...
                    for (int i = 0; i < closureCount; i++)
                    {
                        ...
                        DebugId closureId = new DebugId(closuresBuilder.Count, generation: 0);
                    ...
                    while (blobReader.RemainingBytes > 0)
                    {
                        ...
                        DebugId lambdaId = new DebugId(lambdasBuilder.Count, generation: 0);
