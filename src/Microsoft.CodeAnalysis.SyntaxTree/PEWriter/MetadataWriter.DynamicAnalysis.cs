...
namespace Microsoft.Cci
{
    internal class DynamicAnalysisDataWriter
    {
            ...
            Dictionary<DebugSourceDocument, int> documentIndex)
        {
            ...
            BlobBuilder writer = new BlobBuilder(4 + spans.Length * 4);
        ...
        private BlobHandle SerializeDocumentName(string name)
        {
            ...
            BlobBuilder writer = new BlobBuilder(1 + Math.Max(c1, c2) * 2);

            writer.WriteByte((byte)separator[0]);

            // TODO: avoid allocations
            foreach (string part in name.Split(separator))
            {
        ...
        {
            BlobWriter writer = new BlobWriter(builder.ReserveBytes(_blobHeapSize));
