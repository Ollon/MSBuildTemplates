...
namespace Microsoft.Cci
{
    internal sealed class CustomDebugInfoWriter
    {
        ...
        {
            ArrayBuilder<LocalAndScope> builder = GetLocalInfoToSerialize(
                methodBody,
        ...
        internal static byte[] SerializeCustomDebugMetadata(ArrayBuilder<PooledBlobBuilder> recordWriters)
        {
            ...
            int records = 0;
            foreach(PooledBlobBuilder rec in recordWriters)
            {
                records += rec.Count;
            }
            ...
            BlobWriter cmw = new BlobWriter(result);
            cmw.WriteByte(CustomDebugInfoConstants.Version);
            cmw.WriteByte((byte)recordWriters.Count); // count
            cmw.WriteInt16(0);
            foreach (PooledBlobBuilder recordWriter in recordWriters)
            {
