...
namespace Microsoft.Cci
{
    internal partial class MetadataWriter
    {
        ...
        private BlobHandle SerializeLocalConstantSignature(ILocalDefinition localConstant)
        {
            ...
            // CustomMod*
            CustomModifiersEncoder encoder = new CustomModifiersEncoder(builder);
        ...
        {
            ImmutableArray<TypedConstant> dynamicFlags = local.DynamicTransformFlags;
            ...
            ImmutableArray<TypedConstant> tupleElementNames = local.TupleElementNames;
        ...
        {
            foreach (TypedConstant name in names)
            {
        ...
        {
            if (!index.TryGetValue(document, out DocumentHandle documentHandle))
            {
        ...
        {
            CodeAnalysis.Emit.EditAndContinueMethodDebugInformation encInfo = GetEncMethodDebugInfo(methodBody);
