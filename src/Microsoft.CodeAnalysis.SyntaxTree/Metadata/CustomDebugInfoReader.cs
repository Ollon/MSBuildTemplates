...
namespace Microsoft.CodeAnalysis.Debugging
{
    ...
    internal static class CustomDebugInfoReader
    {
        ...
        {
            foreach (CustomDebugInfoRecord record in GetCustomDebugInfoRecords(customDebugInfo))
            {
        ...
        {
            int offset = 0;
            short numCounts = ReadInt16(bytes, ref offset);

            ArrayBuilder<short> builder = ArrayBuilder<short>.GetInstance(numCounts);
        ...
        public static ImmutableArray<StateMachineHoistedLocalScope> DecodeStateMachineHoistedLocalScopesRecord(ImmutableArray<byte> bytes)
        {
            ...
            ArrayBuilder<StateMachineHoistedLocalScope> builder = ArrayBuilder<StateMachineHoistedLocalScope>.GetInstance(bucketCount);
        ...
        public static ImmutableArray<DynamicLocalInfo> DecodeDynamicLocalsRecord(ImmutableArray<byte> bytes)
        {
            ...
            ArrayBuilder<DynamicLocalInfo> builder = ArrayBuilder<DynamicLocalInfo>.GetInstance(bucketCount);
            for (int i = 0; i < bucketCount; i++)
            {
                ...
                {
                    bool flag = ReadByte(bytes, ref offset) != 0;
                    if (flag)
                    {
                        flags |= 1UL << j;
                    }
                ...
                PooledStringBuilder pooled = PooledStringBuilder.GetInstance();
                StringBuilder nameBuilder = pooled.Builder;
                ...
                string name = pooled.ToStringAndFree();
        ...
        public static ImmutableArray<TupleElementNamesInfo> DecodeTupleElementNamesRecord(ImmutableArray<byte> bytes)
        {
            ...
            ArrayBuilder<TupleElementNamesInfo> builder = ArrayBuilder<TupleElementNamesInfo>.GetInstance(n);
            for (int i = 0; i < n; i++)
            {
        ...
        {
            int n = ReadInt32(bytes, ref offset);
            ArrayBuilder<string> builder = ArrayBuilder<string>.GetInstance(n);
            for (int i = 0; i < n; i++)
            {
                string value = ReadUtf8String(bytes, ref offset);
