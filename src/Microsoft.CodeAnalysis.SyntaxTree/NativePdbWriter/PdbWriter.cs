...
namespace Microsoft.Cci
{
    ...
    internal sealed class PdbWriter : IDisposable
    {
        ...
        public void SerializeDebugInfo(IMethodBody methodBody, StandaloneSignatureHandle localSignatureHandleOpt, CustomDebugInfoWriter customDebugInfoWriter)
        {
            ...
            {
                if (customDebugInfoWriter.ShouldForwardNamespaceScopes(Context, methodBody, methodToken, out IMethodDefinition forwardToMethod))
                {
            ...
            byte[] blob = customDebugInfoWriter.SerializeMethodDebugInfo(Context, methodBody, methodToken, emitEncInfo, suppressNewCustomDebugInfo, out bool emitExternNamespaces);
            if (blob != null)
            {
        ...
        private void DefineNamespaceScopes(IMethodBody methodBody)
        {
            ...
            for (IImportScope scope = namespaceScopes; scope != null; scope = scope.Parent)
            {
                foreach (UsedNamespaceOrType import in scope.GetUsedNamespaces())
                {
                    string importString = TryEncodeImport(import, lazyDeclaredExternAliases, isProjectLevel: false);
            ...
            if (isVisualBasic)
            {
                ...
                {
                    string importString = TryEncodeImport(import, null, isProjectLevel: true);
                    if (importString != null)
                    {
                        UsingNamespace(importString, method);
                    }
        ...
        private string SerializeVisualBasicImportTypeReference(ITypeReference typeReference)
        {
            ...
            PooledStringBuilder result = PooledStringBuilder.GetInstance();
        ...
        private void DefineLocalScopes(ImmutableArray<LocalScope> scopes, StandaloneSignatureHandle localSignatureHandleOpt)
        {
            ...
            ArrayBuilder<LocalScope> scopeStack = ArrayBuilder<LocalScope>.GetInstance();
        ...
        private static object CreateSymWriterWorker()
        {
            ...
            if (!s_MicrosoftDiaSymReaderNativeLoadFailed)
            {
                try
                {
                    Guid guid = new Guid(SymWriterClsid);
                    if (IntPtr.Size == 4)
                    {
        ...
        {
            if (!_documentMap.TryGetValue(document, out ISymUnmanagedDocumentWriter writer))
            {
                ...
                if (!info.Checksum.IsDefault)
                {
                    try
                    {
                        Guid algorithmId = info.ChecksumAlgorithmId;
                        byte[] checksum = info.Checksum.ToArray();
                        uint checksumSize = (uint)checksum.Length;
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        {
            ISymUnmanagedAsyncMethodPropertiesWriter asyncMethodPropertyWriter = _symWriter as ISymUnmanagedAsyncMethodPropertiesWriter;
        ...
        {
            ISymUnmanagedWriter5 writer5 = _symWriter as ISymUnmanagedWriter5;
