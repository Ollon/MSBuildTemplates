...
#pragma warning disable 0649
        ...
        {
            byte[] blobArray = blob.DangerousGetUnderlyingArray();
            fixed (byte* blobPtr = blobArray)
            {
                SnPublicKeyBlob* pkb = (SnPublicKeyBlob*)blobPtr;
                ...
                AlgorithmId signatureAlgorithmId = new AlgorithmId(pkb->SigAlgId);
                if (signatureAlgorithmId.IsSet && signatureAlgorithmId.Class != AlgorithmClass.Signature)
                {
                    return false;
                }

                AlgorithmId hashAlgorithmId = new AlgorithmId(pkb->HashAlgId);
        ...
        {
            SnPublicKeyBlob snPubKey = new SnPublicKeyBlob()
            {
            ...
            using (MemoryStream ms = new MemoryStream(160))
        ...
        public unsafe static bool TryGetPublicKey(ImmutableArray<byte> blob, out ImmutableArray<byte> publicKey)
        {
            ...
            {
                BlobHeader* header = (BlobHeader*)backing;
                RsaPubKey* rsa = (RsaPubKey*)(backing + sizeof(BlobHeader));
