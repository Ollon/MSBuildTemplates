...
namespace Microsoft.CodeAnalysis
{
    ...
    internal class CvtResFile
    {
        ...
        static internal List<RESOURCE> ReadResFile(Stream stream)
        {
            ...
            List<RESOURCE> resourceNames = new List<RESOURCE>();
            ...
            while (stream.Position < stream.Length)
            {
                ...
                RESOURCE pAdditional = new RESOURCE()
                {
                    HeaderSize = cbHdr,
                    DataSize = cbData
                };
    ...
    internal static class COFFResourceReader
    {
        ...
        static internal Microsoft.Cci.ResourceSection ReadWin32ResourcesFromCOFF(Stream stream)
        {
            ...
            SectionHeader rsrc1 = new SectionHeader();
            SectionHeader rsrc2 = new SectionHeader();
            ...
            byte[] imageResourceSectionBytes = new byte[checked(rsrc1.SizeOfRawData + rsrc2.SizeOfRawData)];
            ...
            try
            {
                int relocLastAddress = checked(rsrc1.PointerToRelocations + (rsrc1.NumberOfRelocations * SizeOfRelocationEntry));
            ...
            uint[] relocationOffsets = new uint[rsrc1.NumberOfRelocations];    //offsets into .rsrc$01

            uint[] relocationSymbolIndices = new uint[rsrc1.NumberOfRelocations];
            ...
            try
            {
                long lastSymAddress = checked(peHeaders.CoffHeader.PointerToSymbolTable + peHeaders.CoffHeader.NumberOfSymbols * ImageSizeOfSymbol);
            ...
            MemoryStream outputStream = new MemoryStream(imageResourceSectionBytes);
            BinaryWriter writer = new BinaryWriter(outputStream);  //encoding shouldn't matter. There are no strings being written.
            ...
            for (int i = 0; i < relocationSymbolIndices.Length; i++)
            {
                ...
                long offsetOfSymbol = peHeaders.CoffHeader.PointerToSymbolTable + relocationSymbolIndices[i] * ImageSizeOfSymbol;

                stream.Position = offsetOfSymbol;
                stream.Position += 8; //skip over symbol name
                uint symValue = reader.ReadUInt32();
                short symSection = reader.ReadInt16();
                ushort symType = reader.ReadUInt16();
    ...
    internal static class Win32ResourceConversions
    {
        ...
        {
            BinaryReader iconReader = new BinaryReader(iconStream);

            //read magic reserved WORD
            ushort reserved = iconReader.ReadUInt16();
            if (reserved != 0)
                throw new ResourceException(CodeAnalysisResources.IconStreamUnexpectedFormat);

            ushort type = iconReader.ReadUInt16();
            if (type != 1)
                throw new ResourceException(CodeAnalysisResources.IconStreamUnexpectedFormat);

            ushort count = iconReader.ReadUInt16();
            if (count == 0)
                throw new ResourceException(CodeAnalysisResources.IconStreamUnexpectedFormat);

            ICONDIRENTRY[] iconDirEntries = new ICONDIRENTRY[count];
            ...
            BinaryWriter resWriter = new BinaryWriter(resStream);
        ...
        {
            BinaryWriter resWriter = new BinaryWriter(resStream, Encoding.Unicode);
            resStream.Position = (resStream.Position + 3) & ~3;

            const DWORD RT_VERSION = 16;

            VersionResourceSerializer ver = new VersionResourceSerializer(isDll,
                comments,
                companyName,
                fileDescription,
                fileVersion,
            ...
            long startPos = resStream.Position;
            int dataSize = ver.GetDataSize();
        ...
        private class VersionResourceSerializer
        {
            ...
            internal void WriteVerResource(BinaryWriter writer)
            {
                ...
                */
                ...
                long debugPos = writer.BaseStream.Position;
                ...
                {
                    long writerPos = writer.BaseStream.Position;
