...
namespace Microsoft.CodeAnalysis.SymbolDisplay
{
    internal abstract partial class AbstractSymbolDisplayVisitor : SymbolVisitor
    {
        ...
        private void AddEnumConstantValue(INamedTypeSymbol enumType, object constantValue, bool preferNumericValueOrExpandedFlags)
        {
            ...
            bool isFlagsEnum = IsFlagsEnum(enumType);
            if (isFlagsEnum)
            {
        ...
        private void AddFlagsEnumConstantValue(INamedTypeSymbol enumType, object constantValue, bool preferNumericValueOrExpandedFlags)
        {
            ...
            ArrayBuilder<EnumField> allFieldsAndValues = ArrayBuilder<EnumField>.GetInstance();
            GetSortedEnumFields(enumType, allFieldsAndValues);

            ArrayBuilder<EnumField> usedFieldsAndValues = ArrayBuilder<EnumField>.GetInstance();
            try
            {
            ...
            bool preferNumericValueOrExpandedFlags)
        {
            ...
            ulong constantValueULong = EnumUtilities.ConvertEnumUnderlyingTypeToUInt64(constantValue, underlyingSpecialType);

            ulong result = constantValueULong;
            ...
            if (result != 0)
            {
                ...
                {
                    ulong valueAtIndex = fieldAndValue.Value;
            ...
            ArrayBuilder<EnumField> enumFields)
        {
            ...
            foreach (ISymbol member in enumType.GetMembers())
            {
                if (member.Kind == SymbolKind.Field)
                {
                    ...
                    {
                        EnumField enumField = new EnumField(field.Name, EnumUtilities.ConvertEnumUnderlyingTypeToUInt64(field.ConstantValue, underlyingSpecialType), field);
        ...
        private void AddNonFlagsEnumConstantValue(INamedTypeSymbol enumType, object constantValue)
        {
            ...
            ArrayBuilder<EnumField> enumFields = ArrayBuilder<EnumField>.GetInstance();
            GetSortedEnumFields(enumType, enumFields);

            // See if there's a member with this value.  If so, then use that.
            EnumField match = EnumField.FindValue(enumFields, constantValueULong);
            if (!match.IsDefault)
            {
